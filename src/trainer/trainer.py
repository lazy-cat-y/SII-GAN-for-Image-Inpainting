...
class Trainer():
    def __init__(self, args):
        ...
        self.optimG = optim.Adam(
            self.netG.parameters(), lr=args.lrg, betas=(args.beta1, args.beta2), eps=1e-5
        )

        self.netD = net.Discriminator().cuda()
        self.optimD = optim.Adam(
            self.netD.parameters(), lr=args.lrd, betas=(args.beta1, args.beta2), eps=1e-5
        )
    ...
    def load(self):
        try:
            gpath = sorted(list(glob(os.path.join(self.args.save_dir, 'G*.pt'))))
            num = len(gpath)
            self.netG.load_state_dict(torch.load(gpath[num - 1], map_location='cuda'))
            self.iteration = int(os.path.basename(gpath[num - 1])[1:-3])
            if self.args.global_rank == 0:
                print(f'[**] Loading generator network from {gpath[num - 1]}')
        except:
            pass

        try:
            dpath = sorted(list(glob(os.path.join(self.args.save_dir, 'D*.pt'))))
            num = len(dpath)
            self.netD.load_state_dict(torch.load(dpath[num - 1], map_location='cuda'))
            self.iteration = int(os.path.basename(dpath[num - 1])[1:-3])
            if self.args.global_rank == 0:
                print(f'[**] Loading discriminator network from {dpath[num - 1]}')
        except:
            pass

        try:
            opath = sorted(list(glob(os.path.join(self.args.save_dir, 'O*.pt'))))
            num = len(opath)
            data = torch.load(opath[num - 1], map_location='cuda')
            self.optimG.load_state_dict(data['optimG'])
            self.optimD.load_state_dict(data['optimD'])
            if self.args.global_rank == 0:
                print(f'[**] Loading optimizer from {opath[num - 1]}')
        except:
            pass
    ...
    def save(self):
        if self.args.global_rank == 0:
            ...
            torch.save(
                ...
                os.path.join(self.args.save_dir, f'G{str(self.iteration).zfill(8)}.pt')
            )
            torch.save(
                self.netD.state_dict(),
                os.path.join(self.args.save_dir, f'D{str(self.iteration).zfill(8)}.pt')
            )
            torch.save(
                {'optimG': self.optimG.state_dict(), 'optimD': self.optimD.state_dict()},
                os.path.join(self.args.save_dir, f'O{str(self.iteration).zfill(8)}.pt')
            )
